...
#foxconndf= pd.read_csv('data/STOCK_2883_105_107_FTG.csv', index_col=0 )
foxconndf= pd.read_csv('data/STOCK_CDIB.csv', index_col=0 )
#print(foxconndf)
...
def build_model(input_length, input_dim):
    d = 0.25
    model = Sequential()
    model.add(Conv1D(input_shape=(input_length, input_dim), filters=10, kernel_size=3, padding='valid', activation='relu', strides=1))
